...
class CompareWatch extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchwatch_list: [],
  ...
  search_mobiles = (search) => {
    ...
    this.props.search_mobiles(search_mobiles).then(data => {
      this.setState({ searchwatch_list: data.mobilebrands })
    })
  }
  selection = (e, value) => {
  ...
  render() {
    ...
    const menu = (
      <Menu style={{ background: "#000" }}>
        <Menu.Item><a onClick={() => this.props.history.push('/mobile/compareWatchs')} style={{ color: "#FFF", borderBottom: "solid #FFF 1px" }}>Mobiles</a></Menu.Item>
      ...
      <div style={{ marginLeft: "10%", marginRight: "10%" }}>
        <Row style={{ height: "45px" }}>
          <Col style={{ border: "solid #FFFFFF 1px" }} span={6}>
            <Select
              style={{ width: "100%", padding: "5px" }} placeholder="Please select Watch"
              ...
              {this.state.searchwatch_list ? this.state.searchwatch_list.map(function (item) {
...
export default connect(mapStateToProps, mapDispatchToProps)(CompareWatch);
